                ...
                return (
                  <div key={item.id} className="p-3 rounded-xl border border-border bg-card">
                    <div className="flex items-start gap-3">
                      <div className={`w-9 h-9 rounded-full flex items-center justify-center shrink-0 ${isPendente ? 'bg-amber-500/10' : 'bg-emerald-500/10'}`}>
                        {isPendente ? <Clock size={16} className="text-amber-600" /> : <UserCheck size={16} className="text-emerald-600" />}
                      </div>
                      <div className="flex-1 min-w-0">
                        <p className="text-sm font-semibold text-foreground truncate">{item.nome}</p>
                        <span className={`inline-flex items-center text-[10px] px-1.5 py-0.5 rounded font-semibold mt-0.5 ${isPendente ? 'bg-amber-500/10 text-amber-700' : 'bg-emerald-500/10 text-emerald-700'}`}>
                    ...
                    {!isPendente && resetId === item.id && (
                      <div className="mt-2 p-2 rounded-lg border border-border bg-muted/30 space-y-2">
                        ...
                        <div className="flex gap-1.5">
                          <button onClick={() => { setResetId(null); setResetSenha(''); setResetShow(false); }} className="flex-1 h-9 rounded-lg bg-card border border-border text-xs font-semibold">Cancelar</button>
                          <button onClick={() => resetarSenhaAfiliado(item)} disabled={resetSaving || resetSenha.length < 6} className="flex-1 h-9 rounded-lg bg-primary text-primary-foreground text-xs font-semibold flex items-center justify-center gap-1 disabled:opacity-50">
                            {resetSaving ? <Loader2 size={12} className="animate-spin" /> : <KeyRound size={12} />} Salvar
                          </button>
                        </div>
                      </div>
                    )}
                    ...
                    {isPendente && url && (
                      <div className="mt-2 flex items-center gap-1.5">
                        <div className="flex-1 h-9 px-2.5 bg-muted/50 border border-border rounded-lg flex items-center text-[11px] font-mono text-muted-foreground truncate">{url}</div>
                        <button onClick={() => copiar(url)} className="h-9 px-3 rounded-lg bg-primary text-primary-foreground text-xs font-semibold flex items-center gap-1 active:scale-95">
                          <Copy size={12} /> Copiar
                        </button>
                        <a href={url} target="_blank" rel="noreferrer" className="h-9 w-9 rounded-lg border border-border bg-card flex items-center justify-center text-muted-foreground active:scale-95" title="Abrir">
