...
export default function TabLiderancas({ refreshKey, onSaved, viewOnly }: Props) {
  ...
  useEffect(() => {
    if (cachedData) {
      setData((cachedData as unknown as LiderancaRow[]).filter(l => l.tipo_lideranca !== 'Cabo Eleitoral' && l.tipo_lideranca !== 'Promotor'));
      setLoading(false);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const registroData = {
        ...
        apoiadores_estimados: form.apoiadores_estimados ? (parseInt(form.apoiadores_estimados) || 0) : null,
        meta_votos: form.meta_votos ? (parseInt(form.meta_votos) || 0) : null,
